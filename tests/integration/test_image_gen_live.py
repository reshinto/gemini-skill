"""Live smoke test for `gemini image_gen` — dry-run path.

Image generation is billable per image, so the smoke test verifies the
DRY RUN path instead of actually rendering an image.
...
def test_image_gen_live() -> None:
    result = subprocess.run(
        [sys.executable, str(_RUNNER), "image_gen", "a red square"],

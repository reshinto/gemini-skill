        ...
        "candidates": [{
            "content": {
                "parts": [
                    ...
                    {"executableCode": {"code": "print(2+2)", "language": "PYTHON"}, "id": "exec-1"},
                    {"codeExecutionResult": {"output": "4", "outcome": "OUTCOME_OK"}, "id": "result-1"},
...
class TestCodeExecRun:
    def test_sends_code_execution_tool(self, capsys):
        ...
        with patch("adapters.tools.code_exec.api_call", return_value=_mock_code_response()) as mock_api, \
    ...
    def test_includes_code_in_output(self, capsys):
        from adapters.tools.code_exec import run
        with patch("adapters.tools.code_exec.api_call", return_value=_mock_code_response()), \
             patch("adapters.tools.code_exec.load_config") as mock_cfg:
            mock_cfg.return_value = MagicMock(prefer_preview_models=False, output_dir=None)
            run(prompt="calculate 2+2")
    ...
    def test_text_only_response(self, capsys):
        from adapters.tools.code_exec import run
        with patch("adapters.tools.code_exec.api_call", return_value=_mock_text_only_response()), \
             patch("adapters.tools.code_exec.load_config") as mock_cfg:
            mock_cfg.return_value = MagicMock(prefer_preview_models=False, output_dir=None)
            run(prompt="hello")
    ...
    def test_uses_custom_model(self, capsys):
        from adapters.tools.code_exec import run
        with patch("adapters.tools.code_exec.api_call", return_value=_mock_text_only_response()) as mock_api, \
             patch("adapters.tools.code_exec.load_config") as mock_cfg:
            mock_cfg.return_value = MagicMock(prefer_preview_models=False, output_dir=None)

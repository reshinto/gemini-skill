...
class TestUpdateMain:
    ...
    def test_up_to_date(self, tmp_path, capsys, monkeypatch):
        ...
        with patch.object(
            update_main, "_fetch_latest_release", return_value={"tag_name": "v0.1.0"}
        ):
            update_main.main([])
    ...
    def test_update_available(self, tmp_path, capsys, monkeypatch):
        ...
        with patch.object(
            update_main, "_fetch_latest_release", return_value={"tag_name": "v0.2.0"}
        ):
            update_main.main([])
        output = capsys.readouterr().out
    ...
    def test_fetch_release_fails(self, tmp_path, capsys, monkeypatch):
        ...
        with patch.object(
            update_main, "_fetch_latest_release", side_effect=Exception("network error")
        ):
            update_main.main([])
        output = capsys.readouterr().out

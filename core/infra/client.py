"""REST client for the Gemini API using urllib (stdlib only).
...
def _extract_error_message(error: HTTPError) -> str:
    ...
    try:
        ...
        if "error" in data and "message" in data["error"]:
            return f"Gemini API error ({error.code}): {data['error']['message']}"
    except (json.JSONDecodeError, OSError):
        pass
    return f"Gemini API error: HTTP {error.code} {error.reason}"
